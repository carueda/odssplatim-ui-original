function TimelineWidget(container, tokenForm) {
    ...
    self.platforms = {};   // { platform_id -> some platform info
...
//        'cluster':          true,
    ...
    self.diagnostics = function() {
        console.log("__ tokenInfos (" + tokenInfos.length+ "): ");
        tokenInfos.forEach(function(element, index, array) {
            console.log("__     " + JSON.stringify(element));
        });

        console.log("__ data (" + data.length+ "): ");
...
//        self.timeline.setVisibleChartRange(dr.start, dr.end, true);
    ...
    this.addPlatform = function(platform_id) {

        if (platform_id in platforms) {
            console.log("addPlatform: platform_id already added: " + platform_id);
            return;
        }
    ...
    this.addGroup = function(platform_id) {
        ...
        if (platform_id in platforms) {
            console.log("addGroup: platform_id already added: " + platform_id);
            return;
        }
    ...
    this.addToken = function(tokenInfo) {
        ...
        if (!tokenInfo.platform_id in platforms) {
            self.addPlatform(tokenInfo.platform_id);
        }
...
//            end = new Date(start.valueOf() + dayMs);
    ...
    function formattedGroup(platform_id) {
        console.log("formattedGroup: " + platform_id);
        platform_id = strip(platform_id);
        if (platform_id in platforms) {
            var id      = platforms[platform_id].id;
            var tooltip = platforms[platform_id].name;
            var color = "green";
    ...
    function pushBlock(token, index, push) {
        ...
        if (reallyPush) {
            data.push(body);
            console.log("block pushed: " + JSON.stringify(body));
        }
        else {
            data[index] = body;
            console.log("block updated at index: " +index+ ": " +JSON.stringify(body));
        }
    }
